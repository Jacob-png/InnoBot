...
module.exports = {
    ...
    .setName('weather')
    .setDescription('Get the weather of a given area')
    .addStringOption(option => option.setName('location').setDescription('The location to check the weather of').setRequired(true))
    .addStringOption(option => option.setName('degree-type').setDescription('What degree type you would like').addChoices({ name: `Fahrenheit`, value: 'F'}, {name: `Celsius`, value: 'C'}).setRequired(true)),
    async execute (interaction) {
        ...
        await weather.find({ search: `${location}`, degreeType: `${degree}`}, async function(err, result) {
            ...
            setTimeout(() => {
                if (err) {
                    console.log(err);
                    interaction.editReply({ content: `${err} | Because we are pulling data, sometines timeouts happen! Try again this command`});
                } else {
                    if(result.length == 0) {
